...
class MessageBubble(BoxLayout):
    def __init__(self, text, is_user=True, media_path=None, **kwargs):
        ...
        text_color = WHITE_SOFT if is_user else list(get_color_from_hex("#C0FFA0"))
        lbl = Label(
        ...
        if media_path and os.path.exists(media_path):
            ...
            dl_btn = Button(
                ...
                background_color=GREEN_DIM,
                color=BLACK_PURE, font_size=dp(13)
            )
...
class BigBrainChat(BoxLayout):
    ...
    def _build_ui(self):
        ...
        title = Label(
            text="[b][color=9B30FF]BIG[/color] [color=00FF88]BRAIN[/color] [color=E8D5FF]AI[/color][/b]",
        ...
        self.thinking_label = Label(
            ...
            color=PURPLE_LIGHT, font_size=dp(12)
        )
        self.add_widget(self.thinking_label)
        input_row = BoxLayout(size_hint_y=None, height=dp(56), spacing=dp(6), padding=[dp(8), dp(6)])
        with input_row.canvas.before:
            Color(*BG_MID)
            RoundedRectangle(pos=input_row.pos, size=input_row.size, radius=[0])
        upload_btn = Button(
            text="[+]", size_hint_x=None, width=dp(44),
            size_hint_y=None, height=dp(44),
            background_color=PURPLE_MID, font_size=dp(16)
        )
        ...
        send_btn = Button(
            ...
            color=GREEN_NEON, font_size=dp(16), bold=True
        )
        ...
        def _on_select(inst, selection, *args):
            if selection:
                ...
                try:
                    ...
                    self.uploaded_file = {"name": os.path.basename(path), "content": content, "path": path}
                    self.thinking_label.text = f"Attached: {os.path.basename(path)}"
                except Exception:
                    self.uploaded_file = {"name": os.path.basename(path), "content": "", "path": path}
                popup.dismiss()
